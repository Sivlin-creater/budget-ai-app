<div class="min-h-screen bg-gray-50 dark:bg-neutral-900 rounded">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-8">
            <h3 class="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-4">Overall Budget Summary</h3>
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <a href="/budgets/create"
                class="bg-gradient-to-r from-pink-600 to-rose-600 text-white px-6 py-3 rounded-lg font-semibold hover:shadow-lg transition flex items-center gap-2">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($budgets as $budget)
                    <div class="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-xl transition"
                        ...
                        <!-- Card Header -->
                        <div class="p-6 {{ $budget->category ? '' : 'bg-gradient-to-r from-gray-400 to-gray-600' }}"
                            @if($budget->category)
                        ...
                        <div class="p-6 space-y-4">
                            ...
                            <!-- Optional  -->
                            <button
                                wire:click="toggleDetails({{ $budget->id }})"
                                class="w-full mt-2 text-sm font-semibold text-rose-600 hover:text-rose-800 transition">
                                {{ in_array($budget->id, $expandedBudgets) ? 'Show Less ▲' : 'Show More ▼' }}
                            </button>
                            @if(in_array($budget->id, $expandedBudgets))
                                <div class="mt-4 p-4 bg-gray-50 rounded-lg space-y-3 text-sm">
                                    <div class="flex justify-between">
                                        <span class="text-gray-600">Month</span>
                                        <span class="font-semibold">
                                            {{ \Carbon\Carbon::create($budget->year, $budget->month)->format('F Y') }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-semibold">
                                            ${{ number_format($budget->dailyLimit(), 2) }}
                                        </span>
                                    </div>

                                    <div class="flex justify-between">
                                        <span class="text-gray-600">Avg. Daily Spent</span>
                                        <span class="font-semibold">
                                            ${{ number_format($budget->dailyAverageSpent(), 2) }}
                                        </span>
                                    </div>
                                    ...
                                    <div class="pt-2 border-t text-xs text-gray-500">
                                        @if($budget->is_over)
                                            ⚠️ You have exceeded this budget. Consider adjusting next month.
                                        @elseif($budget->percentage >= 90)
                                            ⚠️ You are very close to the limit.
                                        @elseif($budget->percentage >= 80)
                                            ⚠️ Monitor spending carefully.
                                        @else
                                            ✅ Spending is healthy and on track.
                                        @endif
                                    </div>
                                </div>
                            @endif
        ...
        @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-md p-12 text-center">
                ...
                <a href="/budgets/create"
                    class="inline-flex items-center gap-2 bg-gradient-to-r from-rose-600 to-pink-600 text-white px-8 py-3 rounded-lg font-semibold hover:shadow-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                    </svg>
        ...
        <!-- Tips -->
        @if($budgets->count() > 0)
            <div class="mt-8 bg-pink-50 dark:bg-pink-900 border border-pink-200 dark:border-pink-700 rounded-xl p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h4 class="font-semibold text-rose-900 dark:text-pink-200 mb-3 flex items-center gap-2">
                            Budget Tips
                        </h4>
                        <ul class="text-sm text-pink-800 dark:text-pink-200 space-y-2">
                            <li class="flex items-start gap-2">
                                ...
                                Set realistic budgets based on your historical spending patterns
                            </li>
                            <li class="flex items-start gap-2">
                                <svg class="w-5 h-5 mt-0.5 text-pink-500 dark:text-pink-300 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M5 13l4 4L19 7"/>
                                </svg>
                                Create category-specific budgets for better control
                            </li>
                            <li class="flex items-start gap-2">
                            ...
                            <li class="flex items-start gap-2">
                                ...
                                Enable email alerts to get notified when approaching limits
                            </li>
                        </ul>
                    </div>
                </div>
